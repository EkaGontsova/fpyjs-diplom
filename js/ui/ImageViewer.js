...
class ImageViewer {
  ...
  registerEvents() {
    this.imagesBlock.addEventListener("dblclick", (event) => {
      if (event.target.tagName === "IMG") {
        this.imagePreviewBlock.src = event.target.src; // Отображаем изображение в блоке предпросмотра
      }
    });

    this.imagesBlock.addEventListener("click", (event) => {
      if (event.target.tagName === "IMG") {
        event.target.classList.toggle("selected"); // Меняем класс выделения
        this.checkButtonText(); // Проверяем текст кнопок
      } else if (event.target.classList.contains("select-all")) {
        const images = this.imagesRenderBlock.querySelectorAll("img");
        const isSomeImagesSelected = Array.from(images).some((image) =>
          image.classList.contains("selected")
        );
        images.forEach((image) =>
          image.classList.toggle("selected", !isSomeImagesSelected)
        );
        this.checkButtonText(); // Обновляем текст кнопок
      } else if (event.target.classList.contains("show-uploaded-files")) {
        const modalPreviewer = App.getModal("filePreviewer");
        modalPreviewer.open();
        Yandex.getUploadedFiles((jsonData) =>
          modalPreviewer.showImages(jsonData)
        );
      } else if (event.target.classList.contains("send")) {
        const modalUploader = App.getModal("fileUploader");
        const selectedImages =
          this.imagesRenderBlock.querySelectorAll(".selected");
   ...
   */
  drawImages(images) {
    this.clear(); // Очищаем предыдущие изображения
    if (images.length > 0) {
  ...
  checkButtonText() {
    ...
    const isAllImagesSelected = Array.from(images).every((image) =>
      image.classList.contains("selected")
    );

    this.selectAllBtn.textContent = isAllImagesSelected
      ? "Снять выделение"
      : "Выбрать всё";
